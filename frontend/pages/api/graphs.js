...
export default async function handler(req, res) {
  ...
  // Allow unauthenticated access for initial GET requests (list graphs)
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    if (req.method === 'GET') {
      console.log('Unauthenticated user access - returning empty list');
  ...
  try {
    ...
    const requestOptions = {
      ...
      timeout: 60000 // 60 seconds timeout for graph operations
    };

    // Add body for POST requests
    if (req.method === 'POST') {
      // Log the structure of the request body for debugging
      console.log('Saving graph with structure:',
        JSON.stringify({
    ...
    console.error('Error proxying to graphs API:', error);
    ...
    if (error.type === 'request-timeout') {
      return res.status(504).json({
        ...
        details: 'The request to the backend server took too long to complete. This might be due to the size of the graph or server load.',
